...
class WorkerAgent(CompositionalAgent):
    ...
    def _nlu(self, text, client_id, is_analyze=False):
        # use faq as its core
        ...
        if nlp_faq_response.max_score > self.threshold:
            ###############
            # normal cases: intent is determined by faq doc 
            ###############
            answers = [
            ...
            if len(answers) == 1:
                ###############
                # normal cases (1): single answer; clear intent 
                ###############
                return ActionRequest(
                        client_id=client_id,
                        command=answers[0]['action'],
                        nlu_data={'answers': answers}
                    )
            else:
                ###############
                # normal cases (2): need clarifying action; unclear intent 
                ###############
                return ActionRequest(
                        client_id=client_id,
                        command='clarifying_response()',
                        nlu_data={'answers': answers},
                    )
                    
        else:
            ###############
            # fallout casees: intent=UNK 
            ###############

            return ActionRequest(
                    client_id=client_id,
                    command='fallout_response()',
                )
