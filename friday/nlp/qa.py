...
class ExtractiveQA:
    def __init__(self, model_path: str, document_store_mode: str='es', document_dir: str=None, top_k_retriever: int=10, top_k_reader: int=5):
...
class EmbeddingQA:
    def __init__(self, model_path, top_k_retriever: int=10):
        """
    ...
    def retrieve_top(self, text):
        predictions = self.pipe.run(
            query=text,
            top_k_retriever=self.top_k_retriever
        )
        retrieved = predictions  # temp
        return retrieved
